...
public class CacheUtil {
	...
	/*
	 * 
	 * public static CacheEntry buildCacheEntry(Request req, Connection conn) throws
	 * SQLException {
	 * 
	 * // Parse request to fill the bulk of the CacheEntry object. JsonObject o =
	 * new JsonParser().parse(req.body()).getAsJsonObject(); int userId =
	 * o.get("userId").getAsInt(); String userName =
	 * o.get("userName").getAsString(); int groupId = o.get("groupId").getAsInt();
	 * String groupName = o.get("groupName").getAsString();
	 * 
	 * // Use request info to get a list of all groups associated with the user. //
	 * TODO: Update to retrieve all correct info later. PreparedStatement ps = conn.
	 * prepareStatement("SELECT groupId, groupName FROM belongs_to inner join groups on groupId where belongs_to.userID = ?"
	 * ); ps.setInt(1, userId); ResultSet rs = ps.executeQuery(); ps.close();
	 * 
	 * GroupyWrapper[] groupyWrappers = new GroupyWrapper[getResultSetSize(rs)];
	 * 
	 * int ct = 0; while (rs.next()) { groupyWrappers[ct] = new
	 * GroupyWrapper(rs.getInt(1), rs.getString(2)); ct++; }
	 * 
	...
	public static LoginEntry buildLoginEntry(String userName, int id, Connection conn) throws SQLException {
		...
		PreparedStatement ps = conn.prepareStatement("SELECT *"
				...
				+ " Where tasks.taskID IN (SELECT (taskID) FROM has_tasks WHERE groupID IN (SELECT (groupID) FROM belongs_to WHERE userID = ?));"); // TODO,
																																																																						// Join
																																																																						// relation
																																																																						// table
																																																																						// so
		...
		// TODO, add notification and pinned fields and set them as well? Needs relation
		// table
