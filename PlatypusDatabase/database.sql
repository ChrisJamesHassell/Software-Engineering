...
CREATE DATABASE
IF NOT EXISTS `platypus` /*!40100 DEFAULT CHARACTER SET latin1 */;
USE `platypus`;

-- Dumping structure for table platypus.belongs_to
CREATE TABLE
IF NOT EXISTS `belongs_to`
(
  `groupID` int
...
(11) unsigned DEFAULT NULL,
  PRIMARY KEY
(`groupID`,`userID`),
  KEY `FK_belongs_to_users`
(`userID`),
  CONSTRAINT `FK_belongs_to_group` FOREIGN KEY
(`groupID`) REFERENCES `groups`
(`groupID`) ON
DELETE NO ACTION ON
UPDATE NO ACTION,
  CONSTRAINT `FK_belongs_to_users` FOREIGN KEY
...
-- counter for looping over results
...
IF `counter` > 0
	THEN
WHILE (SELECT COUNT(groupID)
FROM has_events
WHERE groupID = groupIDparam) > 0
		DO
...
(SELECT COUNT(groupID)
FROM has_events
WHERE groupID = groupIDmin);
IF `counter` > 0
			THEN
WHILE (SELECT COUNT(groupID)
FROM has_events
WHERE groupID = groupIDmin) > 0
				DO
SET eventMin
=
(SELECT MIN(eventID)
...
(250) DEFAULT NULL,
  `category` enum
('Appliances','Auto','Meals','Medical','Miscellaneous') NOT NULL,
  `fileName` varchar
(1024) NOT NULL,
...
(`groupID`,`docID`),
  UNIQUE KEY `docID`
(`docID`),
  CONSTRAINT `FK_hasdocuments_documents` FOREIGN KEY
(`docID`) REFERENCES `documents`
(`docID`) ON
DELETE NO ACTION ON
UPDATE NO ACTION,
  CONSTRAINT `FK_hasdocuments_groups` FOREIGN KEY
(`groupID`) REFERENCES `groups`
(`groupID`) ON
DELETE NO ACTION ON
UPDATE NO ACTION
...
(`groupID`,`eventID`),
  UNIQUE KEY `eventID`
(`eventID`),
  CONSTRAINT `FK_hasevents_events` FOREIGN KEY
(`eventID`) REFERENCES `userevents`
(`eventID`) ON
DELETE NO ACTION ON
UPDATE NO ACTION,
  CONSTRAINT `FK_hasevents_groups` FOREIGN KEY
(`groupID`) REFERENCES `groups`
(`groupID`) ON
DELETE NO ACTION ON
UPDATE NO ACTION
...
(`groupID`,`taskID`),
  UNIQUE KEY `taskID`
(`taskID`),
  CONSTRAINT `FK_hastasks_groups` FOREIGN KEY
(`groupID`) REFERENCES `groups`
(`groupID`) ON
DELETE NO ACTION ON
UPDATE NO ACTION,
  CONSTRAINT `FK_hastasks_tasks` FOREIGN KEY
...
(250),
	IN `category` ENUM
('Appliances','Auto','Meals','Medical','Miscellaneous'),
	IN `fileName` VARCHAR
(1024),
...
SET pinned
=
IF(pinned = null, 0, pinned);
-- If pinned is null set to 0, otherwise set to itself

INSERT INTO documents
	(name, description, category, fileName, expirationDate)
VALUES
	(name, description, category, fileName, expirationDate);

SET @docID = last_insert_id();
-- get last inserted task's ID

INSERT INTO has_documents
VALUES
	(
		groupID,
		@docID,
...
END;
    START TRANSACTION;
INSERT INTO users
	(username, firstName, lastName, email, userPassword, dateOfBirth)
VALUES
	(
		username,
		firstName,
...
('Appliances','Auto','Meals','Medical','Miscellaneous') NOT NULL,
  ...
  `priority` enum
('Low','Mid','High') NOT NULL DEFAULT 'Low',
  `completed` binary
(1) NOT NULL DEFAULT '0',
  PRIMARY KEY
(`taskID`)
...
(64) NOT NULL COMMENT 'Changed to 64 length',
